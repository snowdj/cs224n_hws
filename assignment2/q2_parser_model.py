...
class ParserModel(Model):
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        prog = tf.keras.utils.Progbar(target=n_minibatches, interval=1e-3)
...
def main(debug=True, config=Config()):
    print(80 * "=")
    print("INITIALIZING")
    print(80 * "=")
    #config = Config()
    ...
    with tf.Session(graph=graph, config=config.tf_config) as session:
